...
class SimpleGCN(nn.Module):
    ...
    def forward(self, X, adj):  # X: (N, F), adj: (N, N) 0/1 或权重
...
class ClusterNetModel(pl.LightningModule):
    def __init__(self, hparams, input_dim, init_k, feature_extractor=None, n_sub=2, centers=None, init_num=0):
        ...
        # 对于部分 decoder-only 模型，AutoModel 可用；否则可改 AutoModelForCausalLM 并取 base
        ...
        # ===== 节点文本编码：共享 StarCoder 的嵌入（仅取 token embedding）
        ### NEW
        self.token_embed = self.starcoder.get_input_embeddings()  # (vocab, seq_hidden)
        # 简单的节点类型嵌入
        self.node_type_emb = nn.Embedding(8, 32)  # file/line/var/test/... 自己映射到 0..7

        # ===== GCN 图编码器 =====
        ### NEW
        self.graph_encoder = SimpleGCN(in_dim=self.seq_hidden + 32 + 1,  # 文本 + 类型 + sus分数
        ...
        # 将 fused 向量映射到 codes_dim（若不等于 fusion_hidden_dim）
        ### NEW
    ...
    def _encode_seq_batch(self, seq_texts: List[str]):
        """
        用 StarCoder 对序列文本编码，取 mean pooling 得到 (B, seq_hidden)。
        """
    ...
    def _node_type_to_id(self, t: str) -> int:
        ...
        return mapping.get(str(t).lower(), 7)  # 其他

    ### NEW
    def _encode_graph_batch(self, graphs: List[Dict]):
        """
        对每个图：
        - 节点文本 → 用 tokenizer 得到 token ids → 取 token embedding 平均；
        - 拼接 节点类型嵌入 与 sus 分数；
        - 用 GCN 得到每个节点向量 → 再做 mean pool；同时保留节点序列用于注意力。
        返回：
        graph_seq: (B, L_max, Dg) 使用 padding，供跨注意力
        graph_vec: (B, Dg) 图整体向量（mean pool）
        graph_mask: (B, L_max) 有效位
        """
        B = len(graphs)
        ...
        for g in graphs:
            ...
            for nd in nodes:
                ...
                emb = self.token_embed(input_ids).mean(dim=1).squeeze(0)  # (H,)
            ...
            X_text = torch.stack(text_vecs, dim=0)                      # (N, H)
            X_type = self.node_type_emb(torch.tensor(type_ids, device=self.device))  # (N, 32)
            X_sus  = torch.tensor(sus_vals, device=self.device).unsqueeze(-1)        # (N, 1)
            X = torch.cat([X_text, X_type, X_sus], dim=-1)              # (N, H+32+1)
            ...
            for (u, v) in edges:
                ...
                adj[u_idx, v_idx] = 1.0
                adj[v_idx, u_idx] = 1.0  # 无向

            # 3) GCN
            Z = self.graph_encoder(X, adj)         # (N, Dg)
    ...
    def forward(self, batch):
        """
        batch: {"seq_texts": List[str], "graphs": List[dict], "labels": Tensor[B]}
        返回聚类 logits: (B, K)
        """
        if isinstance(batch, (list, tuple)):
            ...
            graph_seq, graph_vec, graph_mask = self._encode_graph_batch(batch["graphs"])  # (B,L,Dg),(B,Dg),(B,L)
            # 将图节点序列（带 mask）作为 KV，序列向量作为 Q 做跨注意力融合
            fused = self.fuser(seq_vec, graph_seq)                         # (B, Df)
            codes = self.proj(fused)                                       # (B, codes_dim)
    ...
    def training_step(self, batch, batch_idx):
        """
        恢复聚类训练流程：用 codes 计算 cluster/subcluster loss（你的原代码已写好，这里调用）。
        """
        # 产出 logits
        logits = self.forward(batch)
        # 产出 codes（用于参数估计等）
        with torch.no_grad():
            # 和 forward 一致的 codes 构造（避免二次计算可做缓存，这里为了清晰直接重算）
        ...
        # === 以下基本恢复你注释掉的 cluster/subcluster loss 计算 ===
        ...
        # 记录 codes/resp/gt 供 epoch_end 用（用原有 log_codes_and_responses）
    ...
    def _pairwise_prf1_jaccard(self, y_true_np, y_pred_np):
        """
        基于列联表的成对统计：TP/FP/FN，然后算 PR/RR/F1/JC/FMI
        y_true_np, y_pred_np: np.ndarray (n,)
        """
        ...
        total_row_pairs = comb2(row_sums)                # 同类对数
        total_col_pairs = comb2(col_sums)                # 同簇对数
        FP = total_col_pairs - TP                         # 同簇不同类
        FN = total_row_pairs - TP                         # 同类不同簇
